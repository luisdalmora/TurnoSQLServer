...
if (!defined('BASE_URL_REDIRECT')) { // Evita redefinir se já definido em outro lugar
...
// Inclui o header ANTES da verificação de admin, pois header.php define isAdmin()
require_once __DIR__ . '/templates/header.php'; 

// Verificação de permissão de administrador APÓS incluir o header
if (!isAdmin()) {
    $_SESSION['flash_message'] = ['type' => 'error', 'message' => 'Você não tem permissão para acessar esta página.'];
    header('Location: ' . BASE_URL . '/home.php'); // Usa BASE_URL do header.php
    exit;
}
...
// Token CSRF para este formulário
$csrfTokenCadColab = $_SESSION['csrf_token_cad_colab'] ?? '';
if (empty($csrfTokenCadColab) && isset($_SESSION['csrf_token_cad_colab'])) {
...
<section class="bg-white p-4 md:p-6 rounded-lg shadow max-w-2xl mx-auto">
    ...
    <form method="POST" action="<?php echo BASE_URL; ?>/processar_cadastro_colaborador.php" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nome_completo" id="nome_completo_cad" required class="form-input block w-full ..." placeholder="Digite o nome completo">
        </div>
        <div>
            <label for="email_cad" class="block text-sm font-medium text-gray-700 mb-1">E-mail (Opcional)</label>
            <input type="email" name="email" id="email_cad" class="form-input block w-full ..." placeholder="exemplo@dominio.com">
        </div>
        <div>
            <label for="cargo_cad" class="block text-sm font-medium text-gray-700 mb-1">Cargo (Opcional)</label>
            <input type="text" name="cargo" id="cargo_cad" class="form-input block w-full ..." placeholder="Ex: Frentista, Caixa">
        </div>
        <div>
            <label for="ativo_cad" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
            <select name="ativo" id="ativo_cad" class="form-select block w-full ..."><option value="1" selected>Ativo</option><option value="0">Inativo</option></select>
        </div>
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 pt-2">
            <a href="<?php echo BASE_URL; ?>/gerenciar_colaboradores.php" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 ...">
                <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i> Voltar
            </a>
            <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 ...">
