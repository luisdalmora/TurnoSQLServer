...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        header("Location: " . SITE_URL . "/conta.html?erro=" . urlencode("Formato de e-mail inválido."));
        if (isset($conexao)) sqlsrv_close($conexao);
        exit;
    }
    ...
    if ($stmt) {
        ...
        if ($novo_usuario_id > 0) {
            ...
            header("Location: " . SITE_URL . "/conta.html?erro=" . urlencode("Erro ao finalizar o cadastro. ID não obtido."));
        }
    } else {
    ...
    header("Location: " . SITE_URL . "/conta.html?erro=" . urlencode("Acesso inválido."));
}
